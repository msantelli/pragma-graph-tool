...
// Generate SVG content string (pure, no browser APIs)
export const generateSVGContent = (diagram: Diagram): string => {
  ...
  const generateSVGTextElement = (
    text: string, x: number, y: number, fontSize: number,
    fill: string, className: string, maxWidth?: number
  ) => {
...
`;
  ...
  sortedNodes.forEach(node => {
    ...
    svgContent += '  ' + generateSVGTextElement(
      node.label, cx, cy, nodeFontSize, textColor, 'node-text', maxWidth
    ) + '\n';
  });
...
// Generate full LaTeX document string (pure, no browser APIs)
...
// Serialize diagram as JSON export string (pure)
...
// Validate imported diagram data (pure)
export const validateDiagramImport = (data: unknown): Diagram => {
  ...
  for (const node of importedData.nodes as Record<string, unknown>[]) {
    if (!node.id || !node.type || !node.position || !node.label) {
      throw new Error('Invalid node structure in diagram');
    }
    if (!['vocabulary', 'practice', 'test', 'operate', 'exit', 'custom'].includes(node.type as string)) {
      throw new Error(`Invalid node type: ${node.type}`);
    }
  }

  for (const edge of importedData.edges as Record<string, unknown>[]) {
  ...
  return {
    id: (importedData.id as string) || Date.now().toString(),
    name: (importedData.name as string) || 'Imported Diagram',
    type: (importedData.type as Diagram['type']) || 'HYBRID',
    nodes: importedData.nodes as Node[],
    edges: importedData.edges as Edge[],
    entryPoints: (importedData.entryPoints as Diagram['entryPoints']) || [],
    exitPoints: (importedData.exitPoints as Diagram['exitPoints']) || [],
    metadata: {
      created: (importedData.metadata as Record<string, string>)?.created || new Date().toISOString(),
      modified: new Date().toISOString(),
      author: (importedData.metadata as Record<string, string>)?.author,
      description: (importedData.metadata as Record<string, string>)?.description
    }
  };
};
